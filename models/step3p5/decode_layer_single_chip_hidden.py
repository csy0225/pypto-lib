...
# -----------------------------------------------------------------------------------------------------------
"""Main 45-layer single-chip decode with a strict raw-hidden boundary.

The only production entry is
``whole_decode_faithful_real_single_chip_hidden_only``. It executes the
complete Main stack in one ``@pl.program`` and emits the pre-final-norm BF16
hidden. vLLM owns final RMSNorm, LM head, target sampling and every
user-visible post-processing operation.

Attention, dense MLP and MoE bodies are inlined into the whole program because
the PyPTO frontend requires orchestration calls to be visible in the enclosing
``@pl.program``. There is no per-layer production dispatcher in this module.
"""
...
assert INTER_S_LOCAL == SHARED_SWIGLU_N_CHUNK * 5
...
# N1 whole-net single-submit hidden-only production entry. The program ends at
# the layer-44 BF16 hidden. Final RMSNorm, LM head, target sampling and every
# user-visible post-processing operation are owned by vLLM.
def _build_whole_decode_faithful_real_single_chip_hidden_only_program(
    *,
    routed_lim: float = 0.0,
    shared_lim: float = 0.0,
    tp_size: int = TP_WORLD_SIZE,
):
    """Build the source-unrolled, single-submit Main 45-layer program.

    Layers 0--2 use the dense full/SWA paths; layers 3--44 use their configured
    full/SWA attention and native-W8A8 MoE paths. Each layer receives its own
    weight row and communication-window slice. The final program output is
    ``next_hidden_out`` only; no final norm, vocabulary projection or sampling
    is present.
    """
    if HIDDEN % tp_size != 0:
        raise ValueError(
    ...
    class WholeDecodeFaithfulRealSingleChipHiddenOnly:
        # Compile-verified inlined attention, dense and MoE method set.
            ...
            for src in pl.range(n_ranks):
                if src != my_rank:
                    pld.system.wait(
                        ...
                        expected=(1 if not MOE_WINDOW_REUSE else moe_epoch), cmp=pld.WaitCmp.Ge,
                    )
        ...
        def whole_chip_orch(  # noqa: PLR0913, PLR0915
            ...
            h_layer_3 = self.swa_moe_chip_orch(
                ...
                dbg_layer_3,
                resid_hold_layer_3,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [0 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [0 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [0 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [0 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [0 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [0 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [0 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [0 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [0 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [0 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                3,
                0,
                my_rank,
                1,  # moe_epoch (C1)
            ...
            h_layer_4 = self.full_moe_chip_orch(
                ...
                dbg_layer_4,
                resid_hold_layer_4,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [1 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [1 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [1 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [1 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [1 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [1 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [1 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [1 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [1 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [1 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                4,
                0,
                2,  # moe_epoch (C1)
                my_rank,
            ...
            h_layer_5 = self.swa_moe_chip_orch(
                ...
                dbg_layer_5,
                resid_hold_layer_5,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [2 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [2 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [2 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [2 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [2 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [2 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [2 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [2 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [2 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [2 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                5,
                3,  # moe_epoch (C1)
                0,
                my_rank,
            ...
            h_layer_6 = self.swa_moe_chip_orch(
                ...
                dbg_layer_6,
                resid_hold_layer_6,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [3 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [3 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [3 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [3 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [3 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [3 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [3 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [3 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [3 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [3 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                4,  # moe_epoch (C1)
                6,
                0,
                my_rank,
            ...
            h_layer_7 = self.swa_moe_chip_orch(
                ...
                dbg_layer_7,
                resid_hold_layer_7,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [4 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [4 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [4 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [4 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [4 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [4 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [4 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [4 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [4 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                5,  # moe_epoch (C1)
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [4 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                7,
                0,
                my_rank,
            )
            ...
            h_layer_8 = self.full_moe_chip_orch(
                ...
                dbg_layer_8,
                resid_hold_layer_8,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [5 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [5 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [5 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [5 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [5 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [5 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [5 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [5 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                6,  # moe_epoch (C1)
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [5 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [5 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                8,
                0,
                my_rank,
            )
            ...
            h_layer_9 = self.swa_moe_chip_orch(
                ...
                dbg_layer_9,
                resid_hold_layer_9,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [6 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [6 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [6 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [6 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [6 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [6 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [6 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                7,  # moe_epoch (C1)
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [6 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [6 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [6 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                9,
                0,
                my_rank,
            )
            ...
            h_layer_10 = self.swa_moe_chip_orch(
                ...
                dbg_layer_10,
                resid_hold_layer_10,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [7 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [7 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [7 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [7 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [7 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [7 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                8,  # moe_epoch (C1)
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [7 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [7 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [7 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [7 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                10,
                0,
                my_rank,
            )
            ...
            h_layer_11 = self.swa_moe_chip_orch(
                ...
                dbg_layer_11,
                resid_hold_layer_11,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [8 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [8 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [8 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [8 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [8 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                9,  # moe_epoch (C1)
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [8 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [8 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [8 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [8 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [8 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                11,
                0,
                my_rank,
            )
            ...
            h_layer_12 = self.full_moe_chip_orch(
                ...
                dbg_layer_12,
                resid_hold_layer_12,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [9 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [9 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [9 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [9 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [9 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                10,  # moe_epoch (C1)
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [9 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [9 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [9 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [9 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [9 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                12,
                0,
                my_rank,
            )
            ...
            h_layer_13 = self.swa_moe_chip_orch(
                ...
                dbg_layer_13,
                resid_hold_layer_13,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [10 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [10 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [10 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [10 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [10 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                11,  # moe_epoch (C1)
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [10 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [10 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [10 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [10 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [10 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                13,
                0,
                my_rank,
            )
            ...
            h_layer_14 = self.swa_moe_chip_orch(
                ...
                dbg_layer_14,
                resid_hold_layer_14,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [11 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [11 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [11 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [11 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [11 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                12,  # moe_epoch (C1)
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [11 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [11 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [11 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [11 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [11 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                14,
                0,
                my_rank,
            )
            ...
            h_layer_15 = self.swa_moe_chip_orch(
                ...
                dbg_layer_15,
                resid_hold_layer_15,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [12 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [12 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [12 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [12 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [12 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                13,  # moe_epoch (C1)
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [12 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [12 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [12 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [12 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [12 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                15,
                0,
                my_rank,
            )
            ...
            h_layer_16 = self.full_moe_chip_orch(
                ...
                dbg_layer_16,
                resid_hold_layer_16,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [13 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [13 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [13 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [13 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                14,  # moe_epoch (C1)
                pl.slice(moe_data_done_stack, [n_ranks, 1], [13 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [13 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [13 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [13 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [13 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [13 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                16,
                0,
                my_rank,
            )
            ...
            h_layer_17 = self.swa_moe_chip_orch(
                ...
                dbg_layer_17,
                resid_hold_layer_17,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [14 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [14 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [14 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [14 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                15,  # moe_epoch (C1)
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [14 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [14 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [14 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [14 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [14 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [14 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                17,
                0,
                my_rank,
            )
            ...
            h_layer_18 = self.swa_moe_chip_orch(
                ...
                dbg_layer_18,
                resid_hold_layer_18,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [15 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [15 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [15 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [15 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                16,  # moe_epoch (C1)
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [15 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [15 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [15 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [15 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [15 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [15 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                18,
                0,
                my_rank,
            )
            ...
            h_layer_19 = self.swa_moe_chip_orch(
                ...
                dbg_layer_19,
                resid_hold_layer_19,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [16 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [16 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [16 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                17,  # moe_epoch (C1)
                pl.slice(moe_count_done_stack, [n_ranks, 1], [16 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [16 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [16 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [16 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [16 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [16 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [16 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                19,
                0,
                my_rank,
            )
            ...
            h_layer_20 = self.full_moe_chip_orch(
                ...
                dbg_layer_20,
                resid_hold_layer_20,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [17 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [17 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                18,  # moe_epoch (C1)
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [17 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [17 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [17 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [17 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [17 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [17 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [17 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [17 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                20,
                0,
                my_rank,
            )
            ...
            h_layer_21 = self.swa_moe_chip_orch(
                ...
                dbg_layer_21,
                resid_hold_layer_21,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [18 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                19,  # moe_epoch (C1)
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [18 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [18 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [18 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [18 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [18 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [18 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [18 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [18 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [18 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                21,
                0,
                my_rank,
            )
            ...
            h_layer_22 = self.swa_moe_chip_orch(
                ...
                resid_hold_layer_22,
                20,  # moe_epoch (C1)
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [19 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [19 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [19 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [19 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [19 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [19 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [19 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [19 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [19 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [19 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                22,
                0,
                my_rank,
            )
            ...
            h_layer_23 = self.swa_moe_chip_orch(
                ...
                21,  # moe_epoch (C1)
                resid_hold_layer_23,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [20 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [20 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [20 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [20 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [20 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [20 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [20 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [20 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [20 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [20 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                23,
                0,
                my_rank,
            )
            ...
            h_layer_24 = self.full_moe_chip_orch(
                ...
                dbg_layer_24,
                resid_hold_layer_24,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [21 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [21 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [21 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [21 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [21 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [21 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [21 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [21 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [21 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [21 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                24,
                0,
                my_rank,
            )
            ...
            h_layer_25 = self.swa_moe_chip_orch(
                ...
                dbg_layer_25,
                resid_hold_layer_25,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [22 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [22 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [22 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [22 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [22 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [22 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [22 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [22 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [22 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [22 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                25,
                0,
                my_rank,
            )
            ...
            h_layer_26 = self.swa_moe_chip_orch(
                ...
                dbg_layer_26,
                resid_hold_layer_26,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [23 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [23 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [23 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [23 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [23 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [23 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [23 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [23 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [23 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [23 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                26,
                0,
                my_rank,
            )
            ...
            h_layer_27 = self.swa_moe_chip_orch(
                ...
                dbg_layer_27,
                resid_hold_layer_27,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [24 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [24 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [24 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [24 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [24 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [24 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [24 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [24 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [24 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [24 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                27,
                0,
                my_rank,
            )
            ...
            h_layer_28 = self.full_moe_chip_orch(
                ...
                dbg_layer_28,
                resid_hold_layer_28,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [25 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [25 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [25 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [25 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [25 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [25 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [25 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [25 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [25 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [25 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                28,
                0,
                my_rank,
            )
            ...
            h_layer_29 = self.swa_moe_chip_orch(
                ...
                dbg_layer_29,
                resid_hold_layer_29,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [26 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [26 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [26 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [26 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [26 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [26 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [26 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [26 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [26 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [26 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                29,
                0,
                my_rank,
            )
            ...
            h_layer_30 = self.swa_moe_chip_orch(
                ...
                dbg_layer_30,
                resid_hold_layer_30,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [27 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [27 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [27 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [27 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [27 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [27 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [27 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [27 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [27 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [27 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                30,
                0,
                my_rank,
            )
            ...
            h_layer_31 = self.swa_moe_chip_orch(
                ...
                dbg_layer_31,
                resid_hold_layer_31,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [28 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [28 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [28 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [28 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [28 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [28 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [28 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [28 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [28 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [28 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                31,
                0,
                my_rank,
            )
            ...
            h_layer_32 = self.full_moe_chip_orch(
                ...
                dbg_layer_32,
                resid_hold_layer_32,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [29 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [29 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [29 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [29 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [29 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [29 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [29 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [29 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [29 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [29 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                32,
                0,
                my_rank,
            )
            ...
            h_layer_33 = self.swa_moe_chip_orch(
                ...
                dbg_layer_33,
                resid_hold_layer_33,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [30 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [30 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [30 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [30 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [30 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [30 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [30 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [30 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [30 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [30 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                33,
                0,
                my_rank,
            )
            ...
            h_layer_34 = self.swa_moe_chip_orch(
                ...
                dbg_layer_34,
                resid_hold_layer_34,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [31 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [31 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [31 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [31 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [31 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [31 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [31 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [31 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [31 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [31 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                34,
                0,
                my_rank,
            )
            ...
            h_layer_35 = self.swa_moe_chip_orch(
                ...
                dbg_layer_35,
                resid_hold_layer_35,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [32 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [32 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [32 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [32 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [32 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [32 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [32 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [32 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [32 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [32 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                35,
                0,
                my_rank,
            )
            ...
            h_layer_36 = self.full_moe_chip_orch(
                ...
                dbg_layer_36,
                resid_hold_layer_36,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [33 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [33 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [33 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [33 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [33 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [33 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [33 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [33 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [33 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [33 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                36,
                0,
                my_rank,
            )
            ...
            h_layer_37 = self.swa_moe_chip_orch(
                ...
                dbg_layer_37,
                resid_hold_layer_37,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [34 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [34 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [34 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [34 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [34 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [34 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [34 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [34 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [34 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [34 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                37,
                0,
                my_rank,
            )
            ...
            h_layer_38 = self.swa_moe_chip_orch(
                ...
                dbg_layer_38,
                resid_hold_layer_38,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [35 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [35 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [35 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [35 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [35 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [35 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [35 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [35 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [35 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [35 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                38,
                0,
                my_rank,
            )
            ...
            h_layer_39 = self.swa_moe_chip_orch(
                ...
                dbg_layer_39,
                resid_hold_layer_39,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [36 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [36 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [36 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [36 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [36 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [36 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [36 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [36 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [36 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [36 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                39,
                0,
                my_rank,
            )
            ...
            h_layer_40 = self.full_moe_chip_orch(
                ...
                dbg_layer_40,
                resid_hold_layer_40,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [37 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [37 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [37 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [37 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [37 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [37 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [37 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [37 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [37 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [37 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                40,
                0,
                my_rank,
            )
            ...
            h_layer_41 = self.swa_moe_chip_orch(
                ...
                dbg_layer_41,
                resid_hold_layer_41,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [38 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [38 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [38 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [38 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [38 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [38 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [38 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [38 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [38 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [38 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                41,
                0,
                my_rank,
            )
            ...
            h_layer_42 = self.swa_moe_chip_orch(
                ...
                dbg_layer_42,
                resid_hold_layer_42,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [39 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [39 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [39 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [39 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [39 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [39 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [39 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [39 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [39 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [39 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                42,
                0,
                my_rank,
            )
            ...
            h_layer_43 = self.swa_moe_chip_orch_swiglu7_silu(
                ...
                dbg_layer_43,
                resid_hold_layer_43,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [40 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [40 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [40 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [40 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [40 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [40 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [40 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [40 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [40 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [40 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                43,
                0,
                my_rank,
            )
            ...
            next_hidden_out = self.full_moe_chip_orch_swiglu7_swiglu16(
                ...
                dbg_out,
                resid_hold_layer_44,
                pl.slice(moe_attn_tmp_stack, [BATCH, HIDDEN], [41 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [41 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [41 * n_ranks * n_ranks if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [41 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [41 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [41 * BATCH if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [41 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [41 * n_routes_per_rank if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [41 * COMM_SIGNAL_STRIDE_I32 if not MOE_WINDOW_REUSE else 0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [41 * local_recv_max if not MOE_WINDOW_REUSE else 0, 0]),
                44,
                0,
                my_rank,
            )
        ...
        def host_orch(  # noqa: PLR0913, PLR0915
            ...
            moe_shared_tmp_stack_buf = pld.alloc_window_buffer(MOE_WINDOW_LAYERS * BATCH * HIDDEN * 2)
            moe_shared_signal_stack_buf = pld.alloc_window_buffer(MOE_WINDOW_LAYERS * COMM_CONTROL_SIGNAL_BYTES)
            ...
            for r in pl.range(pld.world_size()):
                self.whole_chip_orch(
                    ...
                    pld.window(moe_send_route_stack_buf, [MOE_WINDOW_LAYERS * local_recv_max, idx_pad],
                               dtype=pl.INT32),
                    pld.window(moe_shared_tmp_stack_buf, [MOE_WINDOW_LAYERS * BATCH, HIDDEN],
                               dtype=pl.BF16),
                    pld.window(moe_shared_signal_stack_buf, [MOE_WINDOW_LAYERS * COMM_SIGNAL_STRIDE_I32, 1],
                               dtype=pl.INT32),
    ...
    return WholeDecodeFaithfulRealSingleChipHiddenOnly

whole_decode_faithful_real_single_chip_hidden_only = _build_whole_decode_faithful_real_single_chip_hidden_only_program()
