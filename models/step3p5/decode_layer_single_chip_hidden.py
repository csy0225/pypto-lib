    ...
    class WholeDecodeFaithfulRealSingleChipHiddenOnly:
        ...
        def whole_chip_orch(  # noqa: PLR0913, PLR0915
            ...
            h_layer_3 = self.swa_moe_chip_orch(
                ...
                pl.slice(moe_attn_signal_stack, [tp_size, 1], [0, 0]),
                pl.slice(moe_pub_counts_stack, [n_ranks * n_ranks, n_local_experts_pad], [0, 0]),
                pl.slice(moe_count_done_stack, [n_ranks, 1], [0, 0]),
                pl.slice(moe_recv_x_stack, [local_recv_max, HIDDEN], [0, 0]),
                pl.slice(moe_recv_scale_stack, [local_recv_max, 8], [0, 0]),
                pl.slice(moe_data_done_stack, [n_ranks, 1], [0, 0]),
                pl.slice(moe_recv_route_stack, [local_recv_max, idx_pad], [0, 0]),
                pl.slice(moe_send_x_stack, [local_recv_max, HIDDEN], [0, 0]),
                pl.slice(moe_send_scale_stack, [local_recv_max, 8], [0, 0]),
                pl.slice(moe_send_route_stack, [local_recv_max, idx_pad], [0, 0]),
                pl.slice(moe_shared_tmp_stack, [BATCH, HIDDEN], [0, 0]),
                pl.slice(moe_shared_signal_stack, [n_ranks, 1], [0, 0]),
                pl.slice(moe_routed_y_stack, [n_routes_per_rank, HIDDEN], [0, 0]),
                pl.slice(moe_combine_done_stack, [n_ranks, 1], [0, 0]),
                pl.slice(moe_routed_src_stack, [local_recv_max, HIDDEN], [0, 0]),
                3,
                0,
                my_rank,
                1,  # moe_epoch (C1)
