...
def attention_full_prefill(
    ...
    for t in pl.parallel(PREFILL_T):
        ...
        # GM-level flash accumulators — created outside InCore to avoid the
        # Cube fractal-tile reshape error that occurs with [16,1] FP32 tiles
        # created inside pl.at(CORE_GROUP).  Reading them back inside InCore
        # via slice indexing produces MTE (vector) tiles that carry no fractal
        # constraint.
        mi_buf = pl.create_tensor([16, 1], dtype=pl.FP32)
        li_buf = pl.create_tensor([16, 1], dtype=pl.FP32)
        oi_buf = pl.create_tensor([16, HEAD_DIM], dtype=pl.FP32)
        # Per-KV-block intermediates; reused (overwritten) each iteration.
        exp_buf = pl.create_tensor([16, 128], dtype=pl.BF16)
        alpha_buf = pl.create_tensor([16, 1], dtype=pl.FP32)
        beta_buf = pl.create_tensor([16, 1], dtype=pl.FP32)

        # Initialise running accumulators (mi=-inf, li=0, oi=0).
        # pl.full([16, 1], FP32) fails pto.alloc_tile: cols*sizeof = 1*4 = 4 bytes,
        # not 32-byte aligned.  Use pl.full([16, HEAD_DIM], FP32) (128*4=512 bytes,
        # aligned) and derive the [16,1] init via row_max (reduction, no alloc_tile).
        with pl.at(level=pl.Level.CORE_GROUP, name_hint="prefill_full_fa_init"):
            mi_buf = pl.assemble(
                mi_buf,
                pl.row_max(pl.full([16, HEAD_DIM], dtype=pl.FP32, value=-3.0e38)),
                [0, 0],
            )
            li_buf = pl.assemble(
                li_buf,
                pl.row_max(pl.full([16, HEAD_DIM], dtype=pl.FP32, value=0.0)),
                [0, 0],
            )
            oi_buf = pl.assemble(
                oi_buf,
                pl.full([16, HEAD_DIM], dtype=pl.FP32, value=0.0),
                [0, 0],
            )
        ...
        for kh in pl.range(1):
            ...
            for sb in pl.range(ctx_blocks):
                ...
                # Block 1 — QK matmul + online-softmax numerator.
                # Reads mi/li from GM (MTE load, no fractal constraint).
                with pl.at(
                    level=pl.Level.CORE_GROUP,
                    name_hint="prefill_full_fa_qk",
                ):
                    # Padded 16-head block for token t: real [0:8], zero [8:16].
                    ...
                    # Load running mi/li from GM; compute update.
                    mi_cur = mi_buf[0:16, 0:1]
                    li_cur = li_buf[0:16, 0:1]
                ...
                with pl.at(
                    level=pl.Level.CORE_GROUP,
                    name_hint="prefill_full_fa_pv",
                ):
                    exp_b2 = exp_buf[0:16, 0:128]
                    oi_tmp = pl.matmul(exp_b2, v_tile_sb, out_dtype=pl.FP32)
                    oi_cur = oi_buf[0:16, 0:HEAD_DIM]
                    alpha_b2 = alpha_buf[0:16, 0:1]
                    beta_b2 = beta_buf[0:16, 0:1]
                    oi_new = pl.add(
            ...
            with pl.at(
                level=pl.Level.CORE_GROUP,
                name_hint="prefill_full_fa_norm",
            ):
                oi_final = oi_buf[0:16, 0:HEAD_DIM]
                li_final = li_buf[0:16, 0:1]
                ctx = pl.row_expand_div(oi_final, li_final)
                # Slice the 8 real head rows (rows 8-15 are zero-pad).
                ctx_valid = ctx[0:8, 0:HEAD_DIM]
                attn_out_flat = pl.assemble(
                    attn_out_flat,
                    pl.cast(ctx_valid, target_type=pl.BF16),
                    [t * 8 + q_base, 0],
                )
...
def _run_distributed_mock(
    *, layer_idx: int = 0, pass_rate: float = 0.97,
...
if __name__ == "__main__":
    ...
    print(
        f"[OK] built @pl.program PrefillAttentionFull: {program_cls.__name__} "
