...
class WholeDecodeOpt:
    ...
    def whole_chip_orch(  # noqa: PLR0913, PLR0915
        ...
        moe_gate_w: pl.Tensor[[NUM_MOE_LAYERS * HIDDEN, N_EXPERTS], pl.FP32],
        moe_router_bias: pl.Tensor[[NUM_MOE_LAYERS * N_EXPERTS], pl.FP32],
        moe_w_gate_r: pl.Tensor[[NUM_MOE_LAYERS * n_local_experts * HIDDEN, inter], pl.INT8],
        moe_w_gate_r_scale: pl.Tensor[[NUM_MOE_LAYERS * n_local_experts, inter], pl.FP32],
        moe_w_up_r: pl.Tensor[[NUM_MOE_LAYERS * n_local_experts * HIDDEN, inter], pl.INT8],
        moe_w_up_r_scale: pl.Tensor[[NUM_MOE_LAYERS * n_local_experts, inter], pl.FP32],
        moe_w_down_r: pl.Tensor[[NUM_MOE_LAYERS * n_local_experts * inter, HIDDEN], pl.INT8],
        moe_w_down_r_scale: pl.Tensor[[NUM_MOE_LAYERS * n_local_experts, HIDDEN], pl.FP32],
        moe_w_gate_s: pl.Tensor[[NUM_MOE_LAYERS * HIDDEN, sh_inter_local], pl.BF16],
        moe_w_up_s: pl.Tensor[[NUM_MOE_LAYERS * HIDDEN, sh_inter_local], pl.BF16],
        moe_w_down_s: pl.Tensor[[NUM_MOE_LAYERS * sh_inter_local, HIDDEN], pl.BF16],
        seq_lens: pl.Tensor[[USER_BATCH_DYN], pl.INT32],
        block_table: pl.Tensor[[BLOCK_TABLE_FLAT_DYN], pl.INT32],
        slot_mapping: pl.Tensor[[USER_BATCH_DYN], pl.INT32],
        ...
        moe_attn_signal_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * COMM_SIGNAL_STRIDE_I32, 1], pl.INT32
        ],
        moe_pub_counts_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * n_ranks * n_ranks, n_local_experts_pad], pl.INT32
        ],
        moe_count_done_sig_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * n_ranks, 1], pl.INT32
        ],
        moe_recv_x_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * local_recv_max, HIDDEN], pl.INT8
        ],
        moe_recv_scale_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * local_recv_max, 8], pl.FP32
        ],
        moe_data_done_sig_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * n_ranks, 1], pl.INT32
        ],
        moe_recv_r_route_stack: pld.DistributedTensor[
            [NUM_MOE_LAYERS * local_recv_max, idx_pad], pl.INT32
        ],
    ...
    def host_orch(  # noqa: PLR0913, PLR0915
        ...
        moe_gate_w: pl.Tensor[[tp_size, NUM_MOE_LAYERS, HIDDEN, N_EXPERTS], pl.FP32],
        moe_router_bias: pl.Tensor[[tp_size, NUM_MOE_LAYERS, N_EXPERTS], pl.FP32],
        moe_w_gate_r: pl.Tensor[[tp_size, NUM_MOE_LAYERS, n_local_experts, HIDDEN, inter], pl.INT8],
        moe_w_gate_r_scale: pl.Tensor[[tp_size, NUM_MOE_LAYERS, n_local_experts, inter], pl.FP32],
        moe_w_up_r: pl.Tensor[[tp_size, NUM_MOE_LAYERS, n_local_experts, HIDDEN, inter], pl.INT8],
        moe_w_up_r_scale: pl.Tensor[[tp_size, NUM_MOE_LAYERS, n_local_experts, inter], pl.FP32],
        moe_w_down_r: pl.Tensor[[tp_size, NUM_MOE_LAYERS, n_local_experts, inter, HIDDEN], pl.INT8],
        moe_w_down_r_scale: pl.Tensor[[tp_size, NUM_MOE_LAYERS, n_local_experts, HIDDEN], pl.FP32],
        moe_w_gate_s: pl.Tensor[[tp_size, NUM_MOE_LAYERS, HIDDEN, sh_inter_local], pl.BF16],
        moe_w_up_s: pl.Tensor[[tp_size, NUM_MOE_LAYERS, HIDDEN, sh_inter_local], pl.BF16],
        moe_w_down_s: pl.Tensor[[tp_size, NUM_MOE_LAYERS, sh_inter_local, HIDDEN], pl.BF16],
        ...
        # MoE L3-L42 (40 layers) per-layer comm window stacks.
        moe_attn_tmp_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * BATCH * HIDDEN * 2)
        moe_attn_signal_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * COMM_CONTROL_SIGNAL_BYTES)
        moe_pub_counts_stack_buf = pld.alloc_window_buffer(
            NUM_MOE_LAYERS * n_ranks * n_ranks * n_local_experts_pad * 4
        )
        moe_count_done_sig_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * n_ranks * COMM_CONTROL_SIGNAL_BYTES)
        moe_recv_x_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * HIDDEN)
        moe_recv_scale_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * 8 * 4)
        moe_data_done_sig_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * n_ranks * COMM_CONTROL_SIGNAL_BYTES)
        moe_recv_r_route_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * idx_pad * 4)
        moe_send_x_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * HIDDEN)
        moe_send_scale_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * 8 * 4)
        moe_send_route_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * idx_pad * 4)
        moe_sh_tmp_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * BATCH * HIDDEN * 2)
        moe_sh_signal_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * n_ranks * COMM_CONTROL_SIGNAL_BYTES)
        moe_routed_y_buf_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * n_routes_per_rank * HIDDEN * 2)
        moe_combine_done_sig_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * n_ranks * COMM_CONTROL_SIGNAL_BYTES)
        moe_routed_src_buf_stack_buf = pld.alloc_window_buffer(NUM_MOE_LAYERS * local_recv_max * HIDDEN * 2)
        for r in pl.range(pld.world_size()):
            self.whole_chip_orch(
                ...
                pl.reshape(moe_gate_w[r], [NUM_MOE_LAYERS * HIDDEN, N_EXPERTS]),
                pl.reshape(moe_router_bias[r], [NUM_MOE_LAYERS * N_EXPERTS]),
                pl.reshape(moe_w_gate_r[r], [NUM_MOE_LAYERS * n_local_experts * HIDDEN, inter]),
                pl.reshape(moe_w_gate_r_scale[r], [NUM_MOE_LAYERS * n_local_experts, inter]),
                pl.reshape(moe_w_up_r[r], [NUM_MOE_LAYERS * n_local_experts * HIDDEN, inter]),
                pl.reshape(moe_w_up_r_scale[r], [NUM_MOE_LAYERS * n_local_experts, inter]),
                pl.reshape(moe_w_down_r[r], [NUM_MOE_LAYERS * n_local_experts * inter, HIDDEN]),
                pl.reshape(moe_w_down_r_scale[r], [NUM_MOE_LAYERS * n_local_experts, HIDDEN]),
                pl.reshape(moe_w_gate_s[r], [NUM_MOE_LAYERS * HIDDEN, sh_inter_local]),
                pl.reshape(moe_w_up_s[r], [NUM_MOE_LAYERS * HIDDEN, sh_inter_local]),
                pl.reshape(moe_w_down_s[r], [NUM_MOE_LAYERS * sh_inter_local, HIDDEN]),
                seq_lens[r],
                block_table[r],
                slot_mapping[r],
                rope_cos_full[r],
                ...
                pld.window(dense_mlp_signal_stack_buf, [NUM_DENSE_LAYERS * COMM_SIGNAL_STRIDE_I32, 1],
                           dtype=pl.INT32),
                pld.window(moe_attn_tmp_stack_buf, [NUM_MOE_LAYERS * BATCH, HIDDEN],
                           dtype=pl.BF16),
                pld.window(moe_attn_signal_stack_buf, [NUM_MOE_LAYERS * COMM_SIGNAL_STRIDE_I32, 1],
                           dtype=pl.INT32),
                pld.window(moe_pub_counts_stack_buf,
                           [NUM_MOE_LAYERS * n_ranks * n_ranks, n_local_experts_pad],
                           dtype=pl.INT32),
                pld.window(moe_count_done_sig_stack_buf, [NUM_MOE_LAYERS * n_ranks, 1],
                           dtype=pl.INT32),
                pld.window(moe_recv_x_stack_buf, [NUM_MOE_LAYERS * local_recv_max, HIDDEN],
                           dtype=pl.INT8),
                pld.window(moe_recv_scale_stack_buf, [NUM_MOE_LAYERS * local_recv_max, 8],
                           dtype=pl.FP32),
                pld.window(moe_data_done_sig_stack_buf, [NUM_MOE_LAYERS * n_ranks, 1],
                           dtype=pl.INT32),
                pld.window(moe_recv_r_route_stack_buf, [NUM_MOE_LAYERS * local_recv_max, idx_pad],
                           dtype=pl.INT32),
                pld.window(moe_send_x_stack_buf, [NUM_MOE_LAYERS * local_recv_max, HIDDEN],
                           dtype=pl.INT8),
                pld.window(moe_send_scale_stack_buf, [NUM_MOE_LAYERS * local_recv_max, 8],
                           dtype=pl.FP32),
                pld.window(moe_send_route_stack_buf, [NUM_MOE_LAYERS * local_recv_max, idx_pad],
                           dtype=pl.INT32),
                pld.window(moe_sh_tmp_stack_buf, [NUM_MOE_LAYERS * BATCH, HIDDEN],
                           dtype=pl.BF16),
                pld.window(moe_sh_signal_stack_buf, [NUM_MOE_LAYERS * n_ranks, 1],
                           dtype=pl.INT32),
                pld.window(moe_routed_y_buf_stack_buf, [NUM_MOE_LAYERS * n_routes_per_rank, HIDDEN],
                           dtype=pl.BF16),
                pld.window(moe_combine_done_sig_stack_buf, [NUM_MOE_LAYERS * n_ranks, 1],
                           dtype=pl.INT32),
                pld.window(moe_routed_src_buf_stack_buf, [NUM_MOE_LAYERS * local_recv_max, HIDDEN],
                           dtype=pl.BF16),
                r,
                device=r,
            )
