...
* a single ``@pl.jit(auto_scope=False)`` top-level function with a
  ``for layer_idx in pl.range(N)`` runtime loop body (``ForStmt``, not
  ``ForKind::Unroll``); host dispatch is a separate ``@pl.jit.host`` function
  that calls the chip-orch directly per rank (no ``@pl.program`` class);
* per-layer ``chip_orch`` helpers as ``@pl.jit.inline(auto_scope=False)``
  module-level functions with ``pl.Out`` return-params (Var, never a view);
...
explicit pre-loop ``full_chip_orch`` call; L1/L2 share the swa-attn shape and
run inside ``pl.range(2)``. 目标 = 验证判据 A: the DeepSeek loop form
(``pl.range`` + ``@pl.jit.inline`` + ``pl.Out`` return-param) clears codegen
with no return-0 / view-escape walls.
...
whole_decode_opt = whole_chip_orch
