...
"""Production-faithful L0-L4 program for Step3p5 MoE optimization.
...
Both post-layer hidden states are explicit outputs.  ``hidden_l4`` consumes
the actual ``hidden_l3`` tensor, so correctness checks can freeze and compare
both MoE layer boundaries.

The compute bodies are not copied.  This module composes the canonical IR
functions from ``models.step3p5.decode_fwd`` with two focused orchestration
functions.  That keeps attention, dense MLP, router, dispatch, routed/shared
experts, combine, and TP all-reduce identical to the product path.
"""
...
ROUTED_NZ_M0 = _canonical.ROUTED_NZ_M0
ROUTED_NZ_N0 = _canonical.ROUTED_NZ_N0
ROUTED_W13_N1 = _canonical.ROUTED_W13_N1
ROUTED_W13_M1 = _canonical.ROUTED_W13_M1
...
@pl.program
class FiveLayerMoeFocused:
    @pl.function(type=pl.FunctionType.Orchestration)
    def five_layer_chip_orch(  # noqa: PLR0913, PLR0915
        self,
        ...
        moe_w13_r: pl.Tensor[
            [N_MOE_FIVE * n_local_experts * ROUTED_W13_N1, ROUTED_W13_M1, ROUTED_NZ_M0, ROUTED_NZ_N0], pl.INT8
        ],
        moe_w13_r_scale: pl.Tensor[
            [N_MOE_FIVE * n_local_experts, 2 * inter], pl.FP32
        ],
        moe_w_down_r: pl.Tensor[
            [N_MOE_FIVE * n_local_experts * ROUTED_W2_N1, ROUTED_W2_M1, ROUTED_NZ_M0, ROUTED_NZ_N0], pl.INT8
        ],
        ...
        hidden_l4, local_expert_count_l4 = full_moe_chip_orch(
            ...
            pl.reshape(
                pl.slice(
                    moe_w13_r,
                    [n_local_experts * ROUTED_W13_N1, ROUTED_W13_M1, ROUTED_NZ_M0, ROUTED_NZ_N0],
                    [n_local_experts * ROUTED_W13_N1, 0, 0, 0],
                ),
                [n_local_experts, ROUTED_W13_N1, ROUTED_W13_M1, ROUTED_NZ_M0, ROUTED_NZ_N0],
            ),
            pl.slice(
            ...
            pl.reshape(
                pl.slice(
                    moe_w_down_r,
                    [n_local_experts * ROUTED_W2_N1, ROUTED_W2_M1, ROUTED_NZ_M0, ROUTED_NZ_N0],
                    [n_local_experts * ROUTED_W2_N1, 0, 0, 0],
                ),
                [n_local_experts, ROUTED_W2_N1, ROUTED_W2_M1, ROUTED_NZ_M0, ROUTED_NZ_N0],
            ),
            pl.slice(
    ...
    def five_layer_host_orch(  # noqa: PLR0913, PLR0915
        self,
        ...
        moe_w13_r_scale: pl.Tensor[
            [tp_size, N_MOE_FIVE, n_local_experts, 2 * inter], pl.FP32
        ],
        moe_w_down_r: pl.Tensor[
            [
                tp_size,
        ...
        for rank in pl.range(pld.world_size()):
            self.five_layer_chip_orch(
                ...
                pl.reshape(
                    moe_w13_r[rank],
                    [N_MOE_FIVE * n_local_experts * ROUTED_W13_N1, ROUTED_W13_M1, ROUTED_NZ_M0, ROUTED_NZ_N0],
                ),
                pl.reshape(
                    moe_w13_r_scale[rank],
                    [N_MOE_FIVE * n_local_experts, 2 * inter],
                ),
                pl.reshape(
                    moe_w_down_r[rank],
                    [N_MOE_FIVE * n_local_experts * ROUTED_W2_N1, ROUTED_W2_M1, ROUTED_NZ_M0, ROUTED_NZ_N0],
                ),
                pl.reshape(
...
_FOCUSED_PARSED = FiveLayerMoeFocused
_REQUIRED_CANONICAL = (
    "tp_all_reduce",
...
_FUNCTIONS = {
    ...
    for function in _FOCUSED_PARSED.functions.values()
}
