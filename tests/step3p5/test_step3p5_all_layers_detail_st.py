...
from tools.step3p5.pypto_all_layers_detail_compare import compare


DEFAULT_DUMP_ROOT = Path(
    "/mnt/nvme1/chensiyu/logs/step3p5_910b_v017/"
    "vllm_tensor_dump_detail_all_layers_topk"
)
DEFAULT_CKPT_DIR = Path("/mnt/nvme1/chensiyu/step3p5_flash_release_hf_mtp3_bf16")


def test_step3p5_all_layers_detail_tensor_input_st() -> None:
    dump_root = Path(os.environ.get("STEP3P5_VLLM_ALL_DETAIL_DUMP_ROOT", str(DEFAULT_DUMP_ROOT)))
    ckpt_dir = Path(os.environ.get("STEP3P5_CKPT_DIR", str(DEFAULT_CKPT_DIR)))
    if not dump_root.exists():
        pytest.xfail(f"missing vLLM all-layer detail dump root: {dump_root}")
